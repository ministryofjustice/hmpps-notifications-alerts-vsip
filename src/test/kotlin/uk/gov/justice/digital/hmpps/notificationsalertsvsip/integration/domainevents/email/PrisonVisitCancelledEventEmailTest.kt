...
class PrisonVisitCancelledEventEmailTest : EventsIntegrationTestBase() {
  ...
  @Test
  fun `when visit cancelled message is received with outcomeStaus as null then cancelled email is sent`() {
    // Given
    val cancelledVisit = createVisitDto(
      bookingReference = "aa-bb-cc-dd",
      visitDate = LocalDate.now().plusMonths(1),
  ...
  private fun verifyEmailSent(templateId: String, visit: VisitDto, visitAdditionalInfo: VisitAdditionalInfo, templateVars: Map<String, Any>) {
    await untilAsserted { verify(prisonVisitCancelledEventNotifierSpy, times(1)).processEvent(any()) }
    await untilAsserted { verify(visitNotificationService, times(1)).sendMessage(VisitEventType.CANCELLED, visitAdditionalInfo) }
    await untilAsserted { verify(emailSenderService, times(1)).sendVisitsEmail(visit, VisitEventType.CANCELLED, visitAdditionalInfo.eventAuditId, "00000000-0000-0000-0000-000000000002") }
    await untilAsserted {
